...
**Knowledge seeds** in `cypress/knowledge/` — create each with an empty-but-valid shape: `api-catalog.json` (`{"modules": {}}`), `api-behavior-notes.json` (`{"_entryFields": "known_500_bugs/endpoint_quirks entries require endpoint, ticket, recordedAt, lastVerified, recordedBy, note — see .claude/protocols/knowledge-protocol.md", "known_500_bugs": [], "endpoint_quirks": [], "auth_behavior": []}`), `api-dependency-map.json` (`{"modules": {}}`), `failure-patterns.json` (`{"patterns": []}`), `test-run-history.json` (`{"runs": []}`), `agent-run-history.json` (`{"runs": []}` — per-agent wall-clock/step metrics for tuning `maxTurns` budgets from data), `tagging-strategy.json` (`{"@PR": "pre-merge gate", "@Smoke": "post-deploy sanity", "@Regression": "full pass"}`), plus a `_README.md` one-liner pointing at CLAUDE.md's knowledge-base protocol.

**Sample spec** `cypress/e2e/API/health-module/01-get-health.cy.js` — a `@PR @Smoke` GET on `/api/health` (or `/`) following the template skeleton, **plus one `@Regression` negative case** (e.g. a bogus path under the health route asserting 404) so the module participates in the regression gate from day one. Health endpoints are intentionally public, so the spec MUST start with the access-control exemption `// access-control-exempt: health endpoint is intentionally unauthenticated (public liveness probe)` — without it the `access-control` gate (and therefore the pre-commit hook) rejects the scaffold's very first commit. Note: this spec can only prove the wiring once a backend answers — Cypress refuses to start when `baseUrl` is unreachable, so without a running backend the run aborts before any spec executes (Phase 4 must say so plainly, never claim the wiring is proven).
