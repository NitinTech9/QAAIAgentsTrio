...
<!-- Copy this template to cypress/reports/qa-report-{YYYY-MM-DD}.md at the start of every /qa or /qa-only run -->
...
| Fixes applied | N (only for `/qa`) |
| Regression tests added | N (only for `/qa`) |
...
**Commit:** `[sha] fix(qa): [message]` _(only for /qa)_  
**Regression Test Added:** TC-NN _(only for /qa)_  
...
|---|---|---|---|
| `lca-module/` | 12 | High | Not yet implemented |
...
|---|---|---|---|
| `contracts-module` | `03-get-contract-by-id.cy.js` | 2 | No 404 test, no auth test |
| `contract-cancellation-module` | `04-put-cancel-contract.cy.js` | 5 | No DB cleanup |
...
|---|---|---|---|
| TC01 returns 500 | `04-put-cancel-contract.cy.js` | High | App-side DB error — not a test bug |
| `lca-module` not covered | — | High | Not yet implemented — use `/generate-api-test` |
...
2. Run `/generate-api-test` for `lca-module` (12 endpoints, zero coverage)
3. Run `/add-test-cases` on `contracts-module` to add 401/403 and 404 tests
4. Run `/qa-only` after next deploy to confirm no regression

---

*Generated by `/qa` or `/qa-only` skill · TCA Regression Suite QA*
